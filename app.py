...
def _get_paper_index():
    ...
    la._build_paper_index()
    return len(la._qa_paper_index), la._qa_paper_index


@st.cache_data(ttl=300)
def _load_dashboard_data():
    """从 JSON 笔记加载完整 17 字段数据用于仪表盘（缓存 5 分钟）。"""
    papers = []
    notes_dir = la.PDF_DIR / "notes"
    if notes_dir.exists():
...
def _load_config():
    """直接读配置文件，绕过模块缓存。"""
    return json.loads(la.CONFIG_PATH.read_text(encoding="utf-8"))


def _save_config(cfg: dict):
    """保存配置到文件。"""
    la.CONFIG_PATH.write_text(json.dumps(cfg, ensure_ascii=False, indent=2), encoding="utf-8")
    # 刷新模块级 config
    la.config = cfg
...
""", unsafe_allow_html=True)
    ...
    # ── Agent 运行状态指示灯 ──
    try:
        runs_dir = Path(la.BASE_DIR) / "运行日志" / "runs"
    except Exception:
    ...
    if st.session_state.get("_show_session_panel", False):
        if st.button("➕ 新建", use_container_width=True):
            if st.session_state.qa_chat_history:
                ...
                la._save_qa_session(sid, st.session_state.qa_session_name, st.session_state.qa_chat_history)
                ...
                with col_l:
                    if st.button(f"{prefix}{s['name'][:25]} ({s['message_count']})", key=f"l_{s['id']}", use_container_width=True):
                        if st.session_state.qa_chat_history:
                            ...
                            la._save_qa_session(csid, st.session_state.qa_session_name, st.session_state.qa_chat_history)
                        msgs = la._load_qa_session(s["id"])
                        if msgs:
...
if page == "知识库问答":
    ...
    # ── 自动加载最近一次检索简报 ──
    daily_dir = la.OBSIDIAN_DIR / "Daily"
        ...
        with col_save:
            if st.button("保存到 Obsidian", use_container_width=True, help="保存当前对话到 Obsidian QA笔记"):
                if st.session_state.qa_chat_history:
                    ...
                    filepath = la._QA_NOTES_DIR / filename
    ...
    if question := (st.session_state.pop("_auto_question", None) or st.chat_input("输入你的学术问题...")):
        ...
        if use_agent:
            ...
            if plan is None:
                ...
                with st.status(f"执行计划 ({len(plan)} 步)") as status_container:
                    ...
                    step_results = _run_async(la.agent_execute_plan(plan, _progress))

                    # 3. 综合
                    if step_results:
                        st.markdown("---")
                        with st.spinner("🧠 综合所有步骤结果..."):
                            answer = _run_async(la.agent_synthesize(question_augmented, plan, step_results))
                        if answer:
        ...
        if not use_agent:
            ...
            # 概念扩展
            concept_matches = {}
            for cn_term, en_terms in la._CONCEPT_MAP.items():
                if cn_term in question:
                    concept_matches[cn_term] = en_terms
            st.session_state.qa_last_concepts = concept_matches

            # 检索
            la._qa_top_n = st.session_state.qa_top_n
            la._qa_use_external = st.session_state.qa_use_external
            relevant = la._retrieve_papers(question_aug)
            ...
            scores_display = []
            if relevant:
                scored = la._score_and_rank(question_aug, relevant)
                indices = [la._qa_paper_index.index(p) for p in relevant if p in la._qa_paper_index]
                sem_scores = la._semantic_scores(question_aug, indices) if indices else [0]*len(relevant)
            ...
            if not st.session_state.qa_use_external and not relevant:
                ...
                with st.spinner("DeepSeek V4 Pro 思考中..."):
                    messages = la._build_qa_messages(question, relevant, st.session_state.qa_chat_history)
                    answer = _run_async(la._chat_via_deepseek(messages))
                    if not answer:
        ...
        if not st.session_state.qa_agent_mode and relevant and answer:
            warnings = la._verify_answer_claims(answer, relevant, la._qa_paper_index)
            if warnings:
...
Follow-up questions:"""
                        ...
                        ctx_answer = _run_async(la._chat_via_deepseek(ctx_msgs))
                        if ctx_answer:
        ...
        if not st.session_state.qa_session_id:
            ...
            st.session_state.qa_session_name = la._generate_session_name(st.session_state.qa_chat_history)
        la._save_qa_session(sid, st.session_state.qa_session_name, st.session_state.qa_chat_history)
        st.rerun()
            ...
            for i, paper in enumerate(filtered):
                ...
                with st.expander(f"**{title}**  —  {' · '.join(meta_badges)}"):
                    ...
                    with col2:
                        ...
                        if fig_analysis and len(fig_analysis) > 10:
...
# ═══════════════════════════════════════════════════════════════
    ...
    def _do_launch_agent(yr_s, yr_e, tgt_papers, mx_pages, jrnl_filter, jrnl_free, kw_override):
        ...
        try:
            _sp.Popen(
                ...
                stdout=open(log_dir / "worker_stdout.log", "a"),
    ...
    with tab1:
        ...
        if cursor:
            next_kw, scope, ci, ki = la._get_next_keyword(cursor)
            if next_kw:
        ...
        with st.expander("高级搜索", expanded=False):
            ...
            # 期刊过滤
            whitelist_journals = list(la._get_effective_whitelist()) if hasattr(la, '_get_effective_whitelist') else []
...
深入、严谨，触及理论深层逻辑和未解决问题。不要角色扮演，直接输出分析内容。"""
                                ...
                                try:
                                    result = _run_async(la._chat_via_deepseek(msgs))
                                    if result:
                                        st.markdown(result)
                                    else:
...
# ═══════════════════════════════════════════════════════════════
    ...
    with tab1:
        ...
        pdf_files = sorted(la.PDF_DIR.glob("*.pdf")) if la.PDF_DIR.exists() else []
        if not pdf_files:
            st.info("没有 PDF 文件。")
        else:
            # 加载 processed_log
            processed_dois = set()
            if la.PROCESSED_LOG.exists():
                with open(la.PROCESSED_LOG, "r", encoding="utf-8") as f:
            ...
            for pdf in pdf_files:
                doi = la.extract_doi_from_pdf(pdf)
                ...
                with st.expander(f"{emoji} [{conf.upper()}] {item.get('title', '?')[:120]}", expanded=(conf == "low")):
                    ...
                    with col_done:
                        if st.button(f"✅ 已复核，移出队列", key=f"review_done_{i}"):
                            if item.get("doi"):
                                la._remove_from_review_queue(item["doi"])
                    ...
                    with col_retry:
                        if st.button(f"🔄 重新阅读修复", key=f"review_retry_{i}",
                            ...
                            with st.spinner(f"正在修复 {item.get('title','')[:40]}..."):
                                fixed = _run_async(la._retry_fix_notes(
            ...
            if st.button("获取 Zotero 文献列表"):
                with st.spinner("从 Zotero 获取..."):
                    try:
                        ...
                        if coll_key:
                            dois, titles = _run_async(la.fetch_zotero_existing_dois())
    ...
    with tab5:
        ...
        st.json({
            ...
            "processed_log.json": f"{la.PROCESSED_LOG.exists()} ({len(json.loads(la.PROCESSED_LOG.read_text(encoding='utf-8')) if la.PROCESSED_LOG.exists() else [])} 条)",
            "Excel": f"{la.EXCEL_PATH.exists()}",
            "Obsidian Vault": f"{la.OBSIDIAN_DIR.exists()}",
            "新论文待处理": f"{la.PDF_DIR.exists()} ({len(list(la.PDF_DIR.glob('*.pdf'))) if la.PDF_DIR.exists() else 0} 个 PDF)",
            ...
            "影响因子库": f"{len(la._journal_if_map)} 条",
